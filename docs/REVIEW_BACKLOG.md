...
### ⏸️ Deferred (סבב 6)
- **"מה קרה לפריטים?" אחרי קנייה**: אחרי `finishShoppingKeepListActive` הפריטים שנקנו **מוסרים** מהרשימה — הכרטיס "מתכווץ" בלי הסבר. שקלנו copy "נקנו X פריטים בקנייה האחרונה". **חסם:** אין על הכרטיס סיגנל "מתי נקנתה לאחרונה" — צריך join ל-receipt או שדה `lastShoppedAt` על הרשימה. **Trigger:** כשנוסיף סיגנל "קנייה אחרונה" למודל.
- **Empty state מודע-הקשר**: רשימה ריקה מציגה CTA גנרי "הקש להוספת פריטים". רצינו ניסוח חגיגי ("הכל נקנה! 🎉 נוסיף מה שייגמר במזווה"). **חסם:** רשימה ריקה-חדשה ורשימה שרוקנה-אחרי-קנייה נראות זהות על הכרטיס → ניסוח חגיגי יהיה שגוי לרשימה חדשה. דורש אותו סיגנל "קנייה אחרונה" + החלטת מוצר.
