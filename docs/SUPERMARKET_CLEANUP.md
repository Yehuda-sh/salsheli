...
> **סטטוס:** ✅ **כל הפאזות (1-7) הושלמו** (יוני 2026). זה מסמך היסטורי — ראה סעיף הסיום למטה.
...
## למה
המיקוד לסופר ([PRODUCT_DIRECTION](PRODUCT_DIRECTION.md) §1) קבע: **אין בורר סוגים, כל רשימה היא רשימת סופר.** 8 הסוגים האחרים (בית מרקחת, ירקן, אטליז, מאפייה, שוק, משק בית, אירוע, [+ "מי מביא?"]) — נמחקים end-to-end. המסמך המקורי אמר "מוסתרים, הפיכים" — **ההחלטה עודכנה ל"נמחקים".** (PRODUCT_DIRECTION יושר בפאזה 7.)
...
### פאזה 7 — מסמכים
- `PRODUCT_DIRECTION.md`: §1 + טבלת ה-superseded שונו מ"מוסתרים, הפיך" ל**"נמחקו"**.
- `REFACTOR_PLAN.md` + `REVIEW_BACKLOG.md`: שורות המצב "5 קטלוגים מוסתרים/הפיכים" יושרו ל**"נמחקו"**. הלוגים ההיסטוריים נשארו כהיסטוריה.
- `assets/data/README.md`: יושר כבר בפאזה 5. `store-listing.md`: נבדק — אין הפניות לסוגים מרובים.
- עבודה עתידית (לא בוצעה) — ראה למטה.

---

## ✅ כל הפאזות (1-7) הושלמו — יוני 2026

המסמך נשמר כרשומה היסטורית של הניקוי. אפשר למחוק אותו (ההיסטוריה ב-git), אבל סעיף "מחוץ להיקף" למטה הוא המקור היחיד לכך שאישור-הבקשות + פישוט-התפקידים **הושארו במכוון** — לשמר עד שהם ייסגרו.
