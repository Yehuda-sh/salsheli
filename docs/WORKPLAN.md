...
### שלב 2: מערכת קבוצות ✅ הושלם (16/12/2025)
...
- [x] הוספת חברים מאנשי קשר ✅ (16/12/2025)
  - `lib/screens/groups/contact_picker_screen.dart` - מסך בחירת אנשי קשר
  - `lib/services/contact_picker_service.dart` - שירות גישה לאנשי קשר
  - אינטגרציה במסך יצירת קבוצה עם Chips
- [x] מסך פרטי קבוצה ועריכה ✅ (16/12/2025)
  - `lib/screens/groups/group_details_screen.dart` - מסך פרטי קבוצה מלא
  - צפייה בפרטי קבוצה (שם, תיאור, סוג, תאריך יצירה)
  - עריכת שם ותיאור (לבעלים/אדמין)
  - תצוגת תכונות זמינות לפי סוג קבוצה
  - רשימת חברים עם תגי תפקיד צבעוניים
- [x] הגדרת הרשאות לחברים (בעלים/אדמין/חבר) ✅ (16/12/2025)
  - שינוי תפקיד חבר (רק לבעלים)
  - הסרת חבר מקבוצה (לבעלים/אדמין)
  - PopupMenu עם אפשרויות לפי הרשאות
- [x] לוגיקת עזיבת קבוצה ✅ (16/12/2025)
  - Dialog אישור עם הסבר
  - הסרה מהקבוצה וחזרה למסך הקודם
  - בעלים לא יכול לעזוב (חייב למחוק או להעביר בעלות)
- [x] מסך מחיקת קבוצה (רק לבעלים) ✅ (16/12/2025)
  - Dialog אזהרה עם פירוט מה יימחק
  - מחיקת הקבוצה וחזרה למסך הקודם

### שלב 3: מזווה (משפחה בלבד)

#### 3.1: תשתית Firestore ✅ הושלם (16/12/2025)
- [x] יצירת `/users/{userId}/inventory/` למשתמש בודד
- [x] יצירת `/groups/{groupId}/inventory/` לקבוצת משפחה
- [x] עדכון `inventory_repository.dart` (v3.0) - הוספת enum `InventoryLocation` ומתודות חדשות
- [x] עדכון `firebase_inventory_repository.dart` (v4.0) - מימוש מלא user/group inventory
- [x] עדכון `inventory_provider.dart` (v4.0) לתמיכה בשני מיקומים + זיהוי אוטומטי
- [x] לוגיקת מחיקת/העברת מזווה בהצטרפות למשפחה (`transferToGroup`, `deletePersonalInventory`)
- [x] דיאלוג אישור מחיקה/העברה (`inventory_transfer_dialog.dart`)

#### 3.2: עדכון Model - `InventoryItem` ✅ הושלם (16/12/2025)
- [x] הוספת `expiry_date: DateTime?` - תאריך תפוגה
- [x] הוספת `notes: String?` - הערות לפריט
- [x] הוספת `is_recurring: bool` - מוצר קבוע (מתווסף לרשימות חדשות)
- [x] הוספת `last_purchased: DateTime?` - תאריך קנייה אחרון
- [x] הוספת `purchase_count: int` - מספר פעמים שנקנה
- [x] הוספת `emoji: String?` - אמוג'י מותאם (אופציונלי)
- [x] עדכון `toJson()` / `fromJson()` / `copyWith()`
- [x] עדכון `inventory_item.g.dart` (build_runner)
- [x] הוספת getters שימושיים: `isExpired`, `isExpiringSoon`, `daysUntilExpiry`, `isPopular`

#### 3.3: תיקון אמוג'י מיקומים - `StorageLocationsConfig` ✅ הושלם (16/12/2025)
- [x] מקרר: ❄️ → 🧊
- [x] מקפיא: 🧊 → ❄️
- [x] מזווה: 🏠 → 🗄️
- [x] משטח עבודה: 🗄️ → 🍳

#### 3.4: עדכון Dialog עריכה - `PantryItemDialog` ✅ הושלם (16/12/2025)
- [x] הוספת שדה תאריך תפוגה (DatePicker)
- [x] הוספת שדה הערות (TextField)
- [x] הוספת Checkbox "מוצר קבוע"
- [x] הצגת סטטיסטיקות (נקנה X פעמים, קנייה אחרונה)
- [x] עדכון `InventoryProvider.createItem()` לתמיכה בשדות החדשים

#### 3.5: תכונות UI חדשות - `StorageLocationManager` ✅ הושלם (16/12/2025)
- [x] **Empty State** - מסך ריק עם הסבר והכוונה (`pantry_empty_state.dart`)
- [x] **כפתור [🛒] הוסף לרשימה** - ליד פריטים במלאי נמוך (onAddToList callback)
- [x] **Dialog בחירת רשימה** - לאיזו רשימה להוסיף (`select_list_dialog.dart`)
- [x] **תצוגת תאריך תפוגה** - ⏰ עם צבע לפי קרבה (אדום/כתום/ירוק)
- [x] **סינון לפי תפוגה** - הוספה לתפריט סינון (sortBy='expiry')
- [x] **תצוגת מוצר קבוע** - ⭐ ליד שם המוצר (isRecurring)
- [x] **הגדרות מזווה מהירות** - Dialog מ-[⚙️] (`inventory_settings_dialog.dart`)

#### 3.6: התראות מזווה ✅ הושלם (16/12/2025)
- [x] **Dialog מלאי נמוך** - בכניסה לאפליקציה (`low_stock_alert_dialog.dart`)
  - הצגת עד 5 פריטים במלאי נמוך
  - כפתור "הוסף לרשימה" / "למזווה"
  - אפשרות "אל תציג שוב היום"
- [x] **Dialog תפוגה קרובה** - התראה על מוצרים שפג תוקפם (`expiry_alert_dialog.dart`)
  - צבעים לפי דחיפות (אדום - פג, כתום - קרוב)
  - מיון לפי תאריך תפוגה
  - פונקציית `filterExpiringItems` עם threshold

#### 3.7: זיהוי דפוסים ✅ הושלם (16/12/2025)
- [x] לוגיקה לזיהוי מוצר קבוע (`recurring_product_service.dart`)
  - זיהוי מוצרים עם 4+ קניות
  - `findPotentialRecurringProducts()` - מציאת מועמדים
  - `getTopRecurringCandidate()` - המועמד הטוב ביותר
  - `RecurringProductStats` - סטטיסטיקות
- [x] Dialog הצעה להפוך למוצר קבוע (`recurring_product_dialog.dart`)
  - הצגת סטטיסטיקות קנייה
  - כפתורי "הפוך לקבוע" / "לא תודה" / "שאל אחר כך"
- [x] שמירת העדפה "אל תשאל שוב על מוצר זה"
  - `dismissProduct()` / `undismissProduct()`
...
- [x] כפתור "התחל קנייה" (רק למשפחה)
- [x] תצוגת מצב קנייה עם סימון מוצרים
- [x] סימון פריטים: נקנה / אין במלאי / לא צריך
- [x] כפתור "סיים קנייה"
- [x] העברת פריטים שנקנו למזווה
- [x] שמירת פריטים שלא נקנו ברשימה הבאה
- קובץ: `lib/screens/shopping/active/active_shopping_screen.dart`

#### 4.2: רשימת "מי מביא" ✅
- [x] הגדרת `neededCount` לכל פריט
- [x] UI לסימון "אני מביא"
- [x] תצוגת X/Y מתנדבים
- [x] רשימת שמות מתנדבים (עם "..." להרבה)
- [x] חסימה כשמלא (X=Y)
- [x] ביטול סימון
- קובץ: `lib/screens/shopping/who_brings/who_brings_screen.dart`
- שדות במודל: `UnifiedListItem.volunteers`, `neededCount`

#### 4.3: רשימת הצבעה ✅
- [x] תמיכה בתאריך סיום הצבעה
- [x] הצבעה גלויה/חשאית
- [x] UI הצבעה (בעד/נגד/נמנע)
- [x] תצוגת תוצאות (בר התקדמות)
- [x] לוגיקת תיקו והכרעת בעלים
- קובץ: `lib/screens/shopping/voting/voting_screen.dart`
- שדות במודל: `UnifiedListItem.votesFor/Against/Abstain`, `votingEndDate`, `isAnonymousVoting`

#### 4.4: צ'קליסט פשוט ✅
- [x] סימון V פשוט
- [x] סמן הכל / בטל הכל
- [x] בר התקדמות
- [x] זמין לכל סוגי הקבוצות
- קובץ: `lib/screens/shopping/checklist/checklist_screen.dart`
