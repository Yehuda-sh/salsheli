#!/usr/bin/env python3
"""
Sort all catalog files by (category, name) for deterministic order
and easier maintenance. Also drop the 2 known stripped-barcode duplicates
in supermarket (where one row has the canonical 7290-prefixed barcode and
another has only the 8-digit suffix with no brand).
"""
import json
...
def sort_data(data):
    """Sort by (category, name). Hebrew strings collate by Unicode codepoint
...
def drop_stripped_dupes(data):
    """If two items share a name and one has barcode=7290XXXXXXXXX while the
    other has just XXXXXXXXX (the 8-digit suffix) and no brand, drop the
    short-barcode/no-brand row."""
